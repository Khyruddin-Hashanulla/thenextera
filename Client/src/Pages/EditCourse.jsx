  ...
  const fetchCourse = async () => {
    try {
      ...
      if (course.videoLinks && !course.sections) {
        // Convert old videoLinks to sections format
    ...
    try {
      ...
      const processedSections = formData.sections.map((section, index) => {
        ...
        const validVideos = section.videos
          ...
          .map(video => {
            if (!video.title.trim()) {
              throw new Error(`Video title is required in section ${index + 1}`);
            }
            if (!validateVideoUrl(video.url)) {
              throw new Error(`Invalid video URL in section ${index + 1}. Please provide valid YouTube, Vimeo, or direct video URLs.`);
            }
            return {
        ...
        <div className="bg-white/30 backdrop-blur-md shadow-lg rounded-xl p-6">
          ...
          {loading ? (
            <div className="flex justify-center items-center min-h-[400px]">
              <div className="text-xl text-gray-900">Loading course...</div>
            </div>
          ) : (
