...
router.post("/register", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Failed to send verification OTP:", emailError);
      return res.status(500).json({ error: "Failed to send verification OTP. Please try again." });
    }
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user.isEmailVerified) return res.status(400).json({ error: "Please verify your email first. Check your inbox for the verification link.", requiresEmailVerification: true });
...
// Additional routes unchanged for brevity
